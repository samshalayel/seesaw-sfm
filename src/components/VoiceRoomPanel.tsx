...
export function VoiceRoomPanel({ onClose }: Props) {
  ...
  const audioCtxRef      = useRef<AudioContext | null>(null);   // للميكروفون (16kHz)
  const playCtxRef       = useRef<AudioContext | null>(null);   // لتشغيل Gemini (24kHz)
  ...
  // ── تشغيل صوت Gemini (context مستقل بـ 24kHz) ──────────────────────────
  const playAudio = useCallback((base64: string) => {
    // أنشئ context التشغيل عند أول استخدام
    ...
    src.buffer = buf;
    src.connect(ctx.destination);   // مباشرة للسماعات — مستقل عن Agora
