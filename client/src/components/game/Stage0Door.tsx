...
/**
 * Stage0Door — on the LEFT wall (x ≈ −8), z = −3.
 * Opens only when the player is carrying the S0 glass box (carryingBox = true).
 */
export function Stage0Door() {
  const [hovered, setHovered] = useState(false);
  const isLocked       = useGame((s) => s.stage0DoorLocked);
  ...
  return (
    <group position={[-7.92, 2.5, -3]} rotation={[0, Math.PI / 2, 0]}>
      ...
      <group ref={doorGroupRef} position={[doorWidth / 2, 0, 0]}>
        ...
        >
          ...
          <meshStandardMaterial
            color={isLocked ? "#0a0810" : "#0a1628"}
            side={THREE.DoubleSide}
            emissive={hovered ? (isLocked ? "#220811" : "#112244") : "#000000"}
        ...
        {([-0.25, 0.8, -1.3] as number[]).map((y, i) => (
          <mesh key={i} position={[-doorWidth / 2, y, 0.022]}>
            ...
            <meshStandardMaterial
              color={isLocked ? "#881122" : cyan}
              emissive={isLocked ? "#881122" : cyan}
        ...
        <mesh position={[-doorWidth + 0.85, -0.25, 0.04]}>
          ...
          <meshStandardMaterial color={isLocked ? "#993322" : cyan} roughness={0.2} metalness={0.8} />
        </mesh>
        {/* Lock indicator dot */}
      ...
      >
        <div style={{
          background:    hovered ? "#22d3ee" : "rgba(5,12,30,0.96)",
          color:         hovered ? "#050c1e" : "#ffffff",
          ...
          textAlign:     "center",
          whiteSpace:    "nowrap",
          border:        `2px solid ${isLocked ? "#ef4444" : "#22d3ee"}`,
          transition:    "all 0.3s",
          lineHeight:    "1.5",
        }}>
          Stage 1
          <br />
          <span style={{ fontSize: "9px", letterSpacing: "1.5px", color: hovered ? "#050c1e" : "#a0e8f0" }}>
            {isLocked ? "🔒 مقفل" : "Product Shaping"}
          </span>
        </div>
      </Html>}
